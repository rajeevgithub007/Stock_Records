...
def get_resultant_lst(lst):
    result_count = total_buy_sell_count(lst)
    result_max_min = max_min_buy_sell_price(lst)
    result_price=  price_(lst)
    result_quantity= total_buy_sell_quantity(lst)
    ...
    comp_buy_quantity =[{'ISINCurrencyside_':f"{d['ISIN']}|{d['Currency']}|{d['side_']}",
                         'Total Buy Quantity': d['Total Buy Quantity']} for d in result_quantity if d['side_'] == 'BUY']
    comp_sell_quantity =[{'ISINCurrencyside_':f"{d['ISIN']}|{d['Currency']}|{d['side_']}",
                          'Total Sell Quantity': d['Total Sell Quantity']} for d in result_quantity if d['side_'] == 'SELL']
...
def total_buy_sell_quantity(lst):
...
def max_min_buy_sell_price(lst):
    grouper = itemgetter('ISIN', 'Currency', 'side_')
    # print('grouper:', grouper)
    result = []
    for key, grp in groupby(sorted(lst, key=grouper), grouper):
...
def export_to_tsv(filename):
    """ Export the resultant list of dictionaries in tabular format."""
    lst = get_filtered_records(filename)
    ...
    csv_filename = os.path.join(dir_name, 'TSVFile.tsv')
...
if __name__ == '__main__':
    # print('filtered_records: ', get_filter_records(r"C:/Users/Raj/stock_records"))
    filtered_records = get_filtered_records(r"C:/Users/Raj/stock_records")
    print(filtered_records)
    # print('final_list', get_resultant_lst(filtered_records))
    print(export_to_tsv(r"C:/Users/Raj/stock_records"))
